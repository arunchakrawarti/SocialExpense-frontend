...
//     if(res.data.success){
//       localStorage.setItem('expenseLogin', JSON.stringify({
//     user: res.data.user
//   }));
//       props.setlogin(true)
//        navigate('/')
...
const Login = (props) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post('https://fullstackexpense.onrender.com/api/users/login', obj);

      if (res.data.success) {
        // Save both user data and login status in localStorage
        ...
        props.setlogin(true); // Update parent state
